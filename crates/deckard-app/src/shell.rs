...
use deckard_contract::{
    ApprovalStatus, Decision, ExecuteResult, Intent, PendingRecord, Policy, ShieldStatus,
...
/// Background pending-approvals poll cadence (#200): how often the app asks the daemon for the
/// still-pending list while the operator is anywhere BUT the Activity surface (which has its own
/// ~2s feed poller). A few seconds — well inside the ~120s approval TTL — so a dapp or agent
...
/// What the ⌘K "Revoke connected site" command will disconnect (#199). Computed ONCE by
/// [`Shell::connection_revoke_target`] and consumed by BOTH the palette label
/// (`palette::display_label`) and the action (`Shell::run_palette_command`), so the label can never
/// promise a different disconnect than Enter performs — a trust-surface invariant that was
/// previously a hand-maintained partition duplicated across two files.
pub(crate) enum RevokeTarget {
    /// No connected sites — the command is a no-op and shows its static title.
    Nothing,
    /// Exactly one site: disconnect it; the label names its host.
    One(String),
    /// Several sites: disconnect all; the label reads "all N".
    All(usize),
}
...
pub struct Shell {
    ...
    /// Handle to the background pending-approvals poll loop (#200); dropped on lock to cancel it.
    pending_poll_task: Option<gpui::Task<()>>,
    /// The still-pending count from the last background `PendingList` poll — the off-surface
    /// source behind [`Shell::waiting_count`] (the home waiting strip + the sidebar Activity
    /// badge). `None` until the first poll lands (the cues fall back to the last feed snapshot);
    /// cleared on lock so a dead session's count can't linger into the next one.
    polled_pending: Option<usize>,
...
impl Shell {
    ...
    pub fn lock(&mut self, cx: &mut Context<Self>) {
        ...
        self.poll_task = None;
        // Same for the background pending-approvals poll; its snapshot dies with the session
        // (the next unlock re-polls immediately, before a stale cue could render).
    ...
    pub fn stop_revoke_all(&mut self, cx: &mut Context<Self>) {
        ...
        //
        // CANCEL the connections poll here, exactly as `lock` does. Because the bridge clear is
        // sequenced after the daemon revoke round-trip, a still-running poll tick could fetch the
        // not-yet-cleared sessions in that window and — capturing the post-bump epoch — fold them
        // back in, flashing "N connected" onto a locked-down wallet (the panic brake's promise is
        // the row is gone). Dropping the task stops any tick; the next unlock restarts the poll.
        self.connections_poll_task = None;
        self.connections_inflight = false;
        self.connections.clear();
        self.connections_epoch = self.connections_epoch.wrapping_add(1);
    ...
    /// Start the background pending-approvals poll (#200): a low-cadence, whole-session loop
    /// that keeps the "waiting on you" cue live from ANY surface — the home waiting strip and
    /// the sidebar Activity badge — so a dapp or agent request raised while the operator is on
    ...
    fn start_pending_poll(&mut self, cx: &mut Context<Self>) {
        ...
        // moment the shell renders, not one interval later.
        self.kick_pending_refresh(cx);
        self.pending_poll_task = Some(cx.spawn(async move |this, cx| loop {
            cx.background_executor()
                .timer(Duration::from_secs(PENDING_POLL_SECS))
                .await;
            let keep = this.update(cx, |this, cx| {
                let session_live = this.auth == AuthStep::Ready && this.auth_epoch == epoch;
                match pending_poll_tick(session_live, this.surface == Surface::Activity) {
                    PendingPollTick::Stop => false,
                    PendingPollTick::Skip => true,
                    PendingPollTick::Fetch => {
                        this.kick_pending_refresh(cx);
                        true
                    }
                }
            });
    ...
    fn kick_pending_refresh(&mut self, cx: &mut Context<Self>) {
        ...
        self.pending_poll_inflight = true;
        let epoch = self.auth_epoch;
        let client = self.signer.client();
        ...
        cx.spawn(async move |this, cx| {
            ...
            this.update(cx, |this, cx| {
                if this.auth_epoch != epoch {
                    // Lock/re-unlock already reset the in-flight guard — this reply belongs to
                    // a dead session; clearing the NEW session's guard here could stack fetches.
                    return;
                }
    ...
    /// poll is ambient — no needless repaints). On ERROR it KEEPS the current list: `bridge_admin`
    /// already maps "no bridge listening" to an honest empty list, so a genuinely-down bridge
    /// clears; an error here is a reachable-but-unhealthy bridge (a momentary timeout), where
    /// blanking would falsely show "no connected sites" for still-live sessions until the next tick.
    ...
    /// What the ⌘K "Revoke connected site" command targets right now (#199) — the single source
    /// both the palette label and its action read, so they can't disagree about what Enter does.
    /// `pub(crate)` for `palette::display_label`.
    pub(crate) fn connection_revoke_target(&self) -> RevokeTarget {
        match self.connections.as_slice() {
            [] => RevokeTarget::Nothing,
            [only] => RevokeTarget::One(only.origin.clone()),
            many => RevokeTarget::All(many.len()),
        }
    }
    ...
    /// `pub(crate)` so the Connections sidebar row's ✕ (`shell_chrome`) can call it.
    pub(crate) fn revoke_connected_site(&mut self, origin: String, cx: &mut Context<Self>) {
        self.apply_disconnect(move || bridge_admin::revoke_site(&origin), cx);
    }

    /// Disconnect every connected site (#199): the ⌘K "revoke all connected sites" path.
    fn disconnect_all_sites(&mut self, cx: &mut Context<Self>) {
        self.apply_disconnect(bridge_admin::clear_sites, cx);
    }

    /// Run a disconnect op against the bridge and fold its returned live list into
    /// `self.connections` (#199). Shared by per-site revoke and revoke-all so the epoch fence, the
    /// in-flight guard, and the error handling can never drift between the two paths.
    ///
    /// - Bumps `connections_epoch` so an in-flight LIST poll carrying a pre-op snapshot is dropped
    ///   instead of resurrecting a just-revoked row (capture the value AFTER the bump so this op's
    ///   own reply still folds).
    /// - Sets `connections_inflight` so the poll won't START a fresh list fetch under the same
    ///   post-bump epoch while this op round-trips — closing the poll-vs-revoke fence gap the bare
    ///   epoch bump left open.
    /// - On SUCCESS the returned list replaces `self.connections`; on ERROR the current list is
    ///   KEPT and the next poll reconciles. A failed revoke must NOT blank sibling rows or claim
    ///   "all disconnected" — a bridge error is not proof the sessions are gone. (`bridge_admin`
    ///   already turns "no bridge listening" into an honest empty list, so a genuinely-down bridge
    ///   still clears; only a reachable-but-unhealthy bridge yields the error handled here.)
    ///
    /// Accepted edge: two OVERLAPPING revokes whose loopback replies reorder can leave the slower
    /// site's row on screen until the next poll tick reconciles (≤5s) — every revoke IS applied on
    /// the bridge; only the interim display can lag.
    fn apply_disconnect(
        &mut self,
        op: impl FnOnce() -> anyhow::Result<Vec<bridge_admin::ConnectedSite>> + Send + 'static,
        cx: &mut Context<Self>,
    ) {
        let epoch = self.auth_epoch;
        self.connections_epoch = self.connections_epoch.wrapping_add(1);
        ...
        cx.spawn(async move |this, cx| {
            ...
            this.update(cx, |this, cx| {
                if this.auth_epoch != epoch {
                    // Reply belongs to a dead session (lock/re-unlock already reset the guard).
                    return;
                }
                this.connections_inflight = false;
                if this.connections_epoch != cepoch {
                    // A newer disconnect superseded this one; let its reply carry the truth.
                    return;
                }
                // Keep the current list on error (see the doc): only a successful op reshapes it.
                if let Ok(next) = res {
                    if this.connections != next {
                        this.connections = next;
                        cx.notify();
    ...
    pub fn run_palette_command(&mut self, id: &str, window: &mut Window, cx: &mut Context<Self>) {
        ...
        match id {
            ...
            // Disconnect connected dapp site(s) (#199). The action reads the SAME
            // `connection_revoke_target` the palette label rendered, so Enter always disconnects
            // exactly what the label named: nothing / that one site / all of them. Per-site
            // precision at N>1 lives on the sidebar row's ✕; from the palette, revoking all when the
            // label reads "all N" is the fail-safe direction (revoking too much only reduces
            // authority). Session-scoped disconnect, not a ban.
            "revoke-site" => match self.connection_revoke_target() {
                RevokeTarget::Nothing => {}
                RevokeTarget::One(origin) => self.revoke_connected_site(origin, cx),
                RevokeTarget::All(_) => self.disconnect_all_sites(cx),
            },
...
// --- #200: the background pending poll's pure state machine ---
//
// The poll/badge decisions live here as pure functions so the trust-relevant invariants are
// unit-tested: the badge can never contradict the NEEDS YOU band while the feed is on screen,
// the two pollers never double-fetch, and only a still-`Pending` record counts as waiting.

/// What one tick of the background pending poll should do (see [`Shell::start_pending_poll`]).
#[derive(Debug, PartialEq, Eq)]
enum PendingPollTick {
    /// The unlocked session is over (lock or re-unlock bumped the epoch) — end the loop.
    Stop,
    /// The Activity surface is open: its own ~2s feed poller is already refreshing (and
    /// `refresh_activity` reconciles the snapshot) — keep looping, but don't fetch.
    Skip,
    /// Fetch the pending list.
    Fetch,
}

/// Decide one background-poll tick from the session + surface state.
fn pending_poll_tick(session_live: bool, on_activity: bool) -> PendingPollTick {
    if !session_live {
        PendingPollTick::Stop
    } else if on_activity {
        PendingPollTick::Skip
    } else {
        PendingPollTick::Fetch
    }
}

/// The waiting-on-a-human count in a `PendingList` snapshot. ONLY `Pending` counts — `Allowed`
/// no longer needs the human, `Denied`/`Expired` are terminal. The daemon expires stale rows
/// before listing, so a `Pending` here is genuinely inside its approval window.
fn pending_waiting(records: &[PendingRecord]) -> usize {
    records
        .iter()
...
mod pending_poll_tests {
    ...
    /// The two-pollers contract: while Activity is open its own feed poller refreshes, so the
    /// background loop must SKIP the wire call (never double-fetch) — but keep looping, so it
    /// resumes the moment the operator steps off the feed. A dead session ends the loop.
    #[test]
    fn background_poll_skips_while_activity_is_open() {
        assert_eq!(pending_poll_tick(true, false), PendingPollTick::Fetch);
        assert_eq!(pending_poll_tick(true, true), PendingPollTick::Skip);
        assert_eq!(pending_poll_tick(false, false), PendingPollTick::Stop);
        assert_eq!(pending_poll_tick(false, true), PendingPollTick::Stop);
    }

    /// Only a still-`Pending` record is "waiting for you": `Allowed` no longer needs the human,
    /// and `Denied`/`Expired` are settled — counting any of them would cry wolf from the badge.
    #[test]
    fn only_pending_rows_count_as_waiting() {
        let records = vec![
            rec(0x01, ApprovalStatus::Pending),
            rec(0x02, ApprovalStatus::Allowed),
            rec(
                0x03,
            ...
            rec(0x04, ApprovalStatus::Expired),
        ];
        assert_eq!(pending_waiting(&records), 1);
    ...
    /// Off-surface the background snapshot is the truth (#200's whole point): a request that
    /// arrives while the operator is on Home lights the cue even though the feed snapshot is
    /// stale-empty — and a stale feed row that settled can't resurrect the strip.
    #[test]
    fn background_snapshot_wins_off_surface() {
        assert_eq!(waiting_count(false, 0, Some(1)), 1);
        assert_eq!(waiting_count(false, 3, Some(0)), 0);
    }

    /// Before the first background poll lands the last feed snapshot is all there is — and the
    /// zero state keeps both cues quiet (strip shows caught-up, badge hidden).
    #[test]
